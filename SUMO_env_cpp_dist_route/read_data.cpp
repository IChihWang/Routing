...
tuple<double, double> get_Conflict_Region(Car car1, Car car2) {
	...
	if (inter_info.find(test_str) != inter_info.end()) {
		...
		double tau_S1_S2 = 0;
		// --- case 1: from Xm, Ym
		double ans1 = (val_Xm + car1_ptr->length + HEADWAY) / car1_ptr->speed_in_intersection - (val["Ym"]) / car2_ptr->speed_in_intersection + DISTANCE * LANE_WIDTH / car1_ptr->speed_in_intersection;
		// --- case 2: from Xd, Yd
		double ans2 = (val_Xd + car1_ptr->length + HEADWAY) / car1_ptr->speed_in_intersection - (val["Yd"]) / car2_ptr->speed_in_intersection + DISTANCE * LANE_WIDTH / car1_ptr->speed_in_intersection;
		tau_S1_S2 = max(ans1, ans2);

		// Compute tau_S2_S1
		double tau_S2_S1 = 0;
		// --- case 1: from Xm, Ym
		ans1 = (val["Ym"] + car2_ptr->length + HEADWAY) / car2_ptr->speed_in_intersection - (val_Xm) / car1_ptr->speed_in_intersection + DISTANCE * LANE_WIDTH / car2_ptr->speed_in_intersection;
		// --- case 2: from Xd, Yd
		ans2 = (val["Yd"] + car2_ptr->length + HEADWAY) / car2_ptr->speed_in_intersection - (val_Xd) / car1_ptr->speed_in_intersection + DISTANCE * LANE_WIDTH / car2_ptr->speed_in_intersection;
...
double get_Intertime(uint8_t lane, char turn) {
	...
	if (turn == 'S') {
		return road_length / V_MAX;
	}
	else {
		return road_length / TURN_SPEED;
	}
}