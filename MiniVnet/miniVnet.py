...
class MiniVnet:
    ...
    '''
    def choose_car(self, all_cars, to_be_handled_cars, new_cars, choose_car_mode, thread_num):

        #choose_car_mode:
        #    0:  Decide the dependancy of CAV and assign to threads
        #    1:  Random assign CAV from top N intersection to threads    (support multi-thread)
        #    2:  Only route new car              (only one thread supported)
        #    3:  Route/reroute all cars          (only one thread supported)
        #
        ...
        if choose_car_mode == 0:
            ...
            if thread_num == 1:
                ...
                for _, _, car_id_list in self.route_database_list:
                    for car_id in car_id_list:
                        car = all_cars_dict[car_id]
                        if car not in chosen_cars and car in to_be_handled_cars:
                            chosen_cars.append(car)
        ...
        elif choose_car_mode == 2:  # Choose only new cars for routing
            # Only for one thread
            cars_for_threads.append(new_cars)
            for _ in range(thread_num-1):
                cars_for_threads.append([])
        elif choose_car_mode == 3:  # Reroute all cars
            # Only for one thread
            for _ in range(thread_num-1):
                cars_for_threads.append([])
            chosen_cars = to_be_handled_cars.values()
            cars_for_threads.append(chosen_cars)
        else:
